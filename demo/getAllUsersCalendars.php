...
DEMO FOR GETTING CALENDAR ITEMS FROM ALL USERS
...
{
	echo '<a href="getAllUsersCalendars.php">Run</a> -:- ';
	echo '<a href="getAllUsersCalendars.php?erase=1">Reset database</a>';
...
$users = array(
		'hallvard.nygaard@jaermuseet.no',
		'Hanne.Madsen@jaermuseet.no',
	);
...
foreach($users as $user_id => $user_email)
{
	try
	{
		printout('New user: '.$user_email);
		$calendaritems[$user_id] = $cal->getCalendarItems(
